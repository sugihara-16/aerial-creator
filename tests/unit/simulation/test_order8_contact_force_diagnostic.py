...
class _Config:
    ...
    max_cumulative_tangential_slip_m: float = 0.01
...
def test_fixture_can_override_raw_slip_limit_for_fault_isolation_only() -> None:
    source = _Config()
    ...
    config = _fast_config(
        ...
        max_cumulative_slip_m=0.05,
    )

    assert config.max_tangential_slip_speed_mps == 0.05
    assert config.max_cumulative_tangential_slip_m == 0.05
    assert source.max_tangential_slip_speed_mps == 0.02
    assert source.max_cumulative_tangential_slip_m == 0.01
    args = build_parser().parse_args(
        [
            "--max-slip-speed-mps",
            "0.05",
    ...
    assert args.max_cumulative_slip_m == 0.05
