...
def test_provisional_slip_and_contact_loss_are_allowed_until_verified_grasp() -> None:
    ...
    maintained_slip_failure = monitor.observe(
        _observation(
            Order8NaturalContactPhase.LIFT,
            time_s=0.40,
    ...
    assert "selected_contact_slip_speed_limit_exceeded" in (
        maintained_slip_failure.failure_reasons
    )
    result = monitor.finalize()
...
def test_provisional_slip_resets_final_contact_confirmation_dwell() -> None:
    monitor = NaturalContactEvidenceMonitor(Order8NaturalContactConfig())
    ...
    assert provisional.contact_dwell_elapsed_s == 0.0
    assert provisional.grasp_acquired is False
    assert provisional.hard_failure is False
...
def test_cumulative_slip_is_integrated_per_link_and_monitor_reset_clears_state() -> None:
    monitor = NaturalContactEvidenceMonitor(Order8NaturalContactConfig())
    contacts = _selected_contacts(slip_mps=0.02)
    step = monitor.observe(
        _observation(
            Order8NaturalContactPhase.CONTACT_ACQUISITION,
            time_s=0.25,
    ...
    assert step.grasp_acquired is True
    assert step.max_cumulative_tangential_slip_m == 0.0
    ...
    assert step.max_cumulative_tangential_slip_m == pytest.approx(0.005)
    assert step.hard_failure is False
    ...
    assert step.max_cumulative_tangential_slip_m == pytest.approx(0.010)
    assert step.hard_failure is False
    ...
    assert "selected_contact_cumulative_slip_limit_exceeded" in step.failure_reasons

    monitor.reset()
    result = monitor.finalize()
    assert result.attempted is False
    assert result.step_count == 0
    assert result.cumulative_tangential_slip_m_by_link == {}
    assert result.failure_reasons == []
