...
- Source commits: `51bc152` (`[P4-full][Order5] Refine assembly approach control`) and `0180504` (`[P4-full][Orders6-7] Add dynamic dock roundtrip`). This documentation closeout is intentionally separate from both source commits.
...
- Live progress: Dynamic assembly now prints flushed `[dynamic-assembly] simulation_time=... phase=... event=...` lines for every phase transition. User-facing aliases expose `staging`, `axial`, `fixed`, `unload`, and `separation`; the synchronous outer runner concurrently drains both child streams and forwards those progress lines immediately instead of hiding them behind final JSON capture.
...
- Tests added or run: The final focused assembly/controller/simulation/hashing regression, including fallback-default and live-progress forwarding coverage, passed `162` in `1.96 s` under `isaaclab3`; both formal seed-2 real-Isaac commands exited zero and passed their typed validators. Python compilation and whitespace checks pass at closeout.
...
- Tests passed: Final focused assembly/controller/simulation/hashing suite `162 passed in 1.96 s`; formal real-Isaac attach-only and roundtrip commands both exited zero. Python compilation and whitespace checks pass at closeout.
