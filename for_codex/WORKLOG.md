...
- Full-task evidence: Under the same fixed eight-seed bucket-9 subset, update 24 passed `0/8` with eight object drops, update 32 passed `0/8` with seven drops and one timeout, update 40 passed `0/8` with eight timeouts, and the most promising late checkpoint update 44 passed `0/8` with eight timeouts.  Update 40 passed bucket 8 at `8/8`; update 45's existing first-eight formal rows pass bucket 8/9 at `8/8` and `2/8`, yielding `0.625` versus update 40's `0.5` and the at-most `0.5` of the other checked intermediates.
- Diagnosis: The bucket-9 terminal trend improves from contact loss and approximately `0.101 m` mean position error at update 24, through approximately `0.067 m` at update 32, to both contacts retained and approximately `0.037 m` position / `0.551 rad` orientation error at update 40.  Update 45 is the best observed checkpoint and produces some complete tasks, but still fails primarily on orientation.  The evidence does not support final-checkpoint selection as the rejection cause; it supports incomplete bucket-9 orientation generalization at the exhausted quota.
- Files changed: `amsrr/simulation/order9_object_task_runtime.py`, `scripts/order9_vectorized_isaac_rollout.py`, `tests/unit/simulation/test_order9_object_task_runtime.py`, this worklog, and the design supplement.  Generated raw/JSONL/log/summary evidence remains ignored under the stage `checkpoint_audit` directory.
- Schema/interface changes: No persisted policy, dataset, checkpoint, reward, controller, safety, or acceptance schema changed.  Two diagnostic-only rollout CLI options and additive provenance metadata were introduced; promotion use fails closed.
- Upstream/downstream impact: Uses the existing hash-bound canonical phase reset bank, final checkpoint lineage, fixed validation buckets, and real-Isaac task runtime.  C2 remains rejected, the `0.85` gate is unchanged, and C3 remains blocked.  No new checkpoint-selection rule, additional PPO budget, or curriculum/reward change is implied.
- Verification/commands: Python compilation and `git diff --check` passed; focused runtime/task/artifact tests passed `15`; the complete unit suite passed `1198` with `1` skip in `94.12 s`.  Real-Isaac phase-isolated checks completed `2 x 100` environments for direct transport and `2 x 100` for lift-to-transport.  Full-task checks completed update-24/32/40/44 bucket-9 subsets and update-40 bucket 8; mathematically dominated in-progress bucket-8 diagnostics were stopped without artifacts.  The ignored machine-readable audit summary validates as JSON and has SHA-256 `6b1e0a337c6385923e3c1be23a230b6ae03c640203b4135b96b0a19ff418a362`.
- Assumptions/limitations/blocker: Representative updates `24/32/40`, the best screened late intermediate `44`, and final `45` were checked; this is not an exhaustive full-task run of all 46 checkpoints.  The existing method-level blocker remains unchanged: proceeding requires approval for additional on-policy budget or a training/reward/curriculum response to orientation generalization, not a silent checkpoint swap.
- Next step: Validate and hash the audit summary, run the complete unit suite, then stop for the existing method decision; do not start C3.

### 2026-07-23 (Order 9 C2 quota completion and rejected promotion)
- Spec/work package: `A-MSRR_codex_ready_spec_v0_4_ja.md` v0.4 plus the approved Order 9 supplements; Agent J/K fixed-morphology conservative `pi_L` PPO execution and promotion evaluation.
- Training result: Accepted exact-replay updates `0--45` are contiguous and consumed `46 x 65,536 = 3,014,656` recorded train-plus-validation interactions, satisfying the configured `3,000,000` quota.  The final update completed `4/4` epochs and `44` optimizer steps without KL early stopping; actor/value/total losses were `0.00135848 / 58.2931 / 29.1793`, mean KL/clip fraction was `0.00611420 / 0.0712921`, and checkpoint SHA-256 is `1378d17d09d1aaa3a2e7fbf178ff6f7359432a46669243d943aeba3203c5476a`.
- Training runtime/load: The 92 immutable train/validation rollouts accumulated `515.7995 s` of rollout-only time and measured `5844.63 env-step/s`.  Sample-weighted collector GPU utilization was `69.18%`, with `99%` peak utilization, `12,337 MiB` peak device memory, `57 C` peak temperature, and `134.07 W` peak power.  Updates `25--45` used the deterministic gzip-level-1 writer and averaged `539.94 s` per complete generation (`116.26 s` collection command, `244.23 s` dataset build, and `173.35 s` PPO update).
- Formal evaluation: Ran two concurrent deterministic, phase-zero, first-terminal full-mesh real-Isaac evaluations with the final checkpoint, 100 environments/episodes per validation bucket, and no runtime fallback.  Validation bucket 8 passed `100/100`; bucket 9 passed `24/100`; aggregate success and no-fallback success were therefore `124/200 = 0.62`.  Safety-failure episodes and fallback decisions were both zero.  Bucket 8 raw/JSONL SHA-256 values are `62a716fcc26759c8018c4527346938e18dcb1e25fd3a6b7e05a4122dee72fbe2` / `b22b0f0f6e2a112e6a4f54d76ca8812029f61525c2e2daf7dc5ab899c98cae68`; bucket 9 values are `b45a68c3ee9544292b46b99153d1ff2511b4f2af7da3254d9b84197c2c65f6ac` / `70a64f1be78adb5c067bcbf97d347707f78bb677e606afc64717330ceab6e8c0`.
- Failure diagnosis: Bucket 9 produced `70` transport, `4` place, and `2` release timeouts.  At the 70 transport terminals, both selected contacts were still active (minimum selected forces `2.94--3.16 N`) and object position error averaged `0.02754 m`, below the unchanged `0.05 m` gate.  Object orientation error was instead `0.2230--0.6407 rad` (mean `0.3219 rad`), above the unchanged `0.20 rad` gate.  The raw artifact is bound to `planned_phase_goal_v1` and the final checkpoint, so this is measured condition-specific orientation-hold/generalization failure rather than the corrected successor-reference defect, a contact drop, a safety terminal, or an evaluation-row mismatch.
- Promotion result/artifacts: The hash-verified report records `200` episodes, success rate `0.62`, no-fallback success rate `0.62`, zero safety/fallback failures, and `5844.63 env-step/s`.  Formal finalization rejected C2 only on `minimum_success_rate` (`0.85` required); the evaluation-report/rejected-manifest SHA-256 values are `aecbbb089901bb674eb63680d3f31a9ebff2a19c04ae7476f66f1d22b34fb1ae` / `7e68c1f62fd2da2fb9e5400c8957115a2e572df56127056d194be75f7f54dbea`.  C2 is not promoted and C3 must not start from this result.
- Files/schema/interfaces: Only this worklog is changed in the repository; generated rollout, checkpoint, TensorBoard, report, and rejection artifacts remain ignored but hash-bound under `artifacts/p4_full/order9/stages/c2_pi_l_ppo_fixed_conservative/`.  No schema, policy observation/action, QPID/QP, reward, safety, randomization, phase gate, promotion threshold, or controller interface changed.
- Verification/commands: Both Isaac evaluators exited zero.  Direct JSONL aggregation, SHA-256 verification, memory-mapped raw-tensor diagnosis, `order9_stage.py build-evaluation`, and `order9_stage.py finalize` passed.  Focused evaluation/curriculum/pipeline/tensor-artifact/reward/task regression passed `21` tests; the post-phase-reference complete unit suite had already passed `1197` with `1` skip.
- Blocker/next step: The configured quota is exhausted and the configured physical promotion gate failed.  Continuing now requires an explicit method decision among validation-based checkpoint selection/audit, a bounded additional on-policy budget, or a change to training/reward/curriculum coverage for object orientation.  Do not lower the physical success threshold or silently proceed to C3.  Work stops here as required for a method-level undefined choice.

### 2026-07-23 (Order 9 C2 update 24 validation and phase-reference correction)
- Training checkpoint: Accepted updates `0--24` are contiguous and exact-replay verified, totaling `25 x 65,536 = 1,638,400` recorded train-plus-validation interactions.  Update 24 completed `4/4` epochs and `44` minibatches without KL early stop; actor/value/total losses were `-0.00143796 / 58.9766 / 29.5182`, mean KL/clip fraction was `0.00704992 / 0.0901449`, and checkpoint SHA-256 is `2eb1f60b8481d5a0067f6bad33eff32ab58cc84f84c308f1775a6560232c827a`.
- Periodic full-task result before correction: The configured eight-episode deterministic validation check reached approach/contact/lift/transport/place without fallback, collision, drop, or QP-infeasible terminal, but timed out `0/8` at place.  The object was already resting at the intended support height while its accumulated target was physically below the support.  Raw/evaluation SHA-256 values are `2d472968d9e7a8b90da67b3aa62f3f7125d454f8364d5e8bd6da08167bbee6c4` and `ebb5ebf14a4f4328f4206e25b62e498489a5275a983a7b49c03fd7585092c26b`.
- Diagnosis/control: The rollout had re-anchored each successor phase to the observed post-success state, causing allowed tracking error to accumulate across phases.  A promoted-C1 control run under the same C2 adapter, validation bucket 8, and seeds `9019--9026` also failed `0/8`, at lift rather than place; raw/evaluation SHA-256 values are `060ff6072fb6f83d42d7c6912bbed9ebc8e6a9e7efc4be1a8f25422ab746b29c` and `bf3ccb81e2708592a685be0ead61797beffe852413fe7e42862a363f987cb8c3`.  This excludes PPO collapse and identifies an inherited runtime-continuity defect.
- Correction: Successful in-episode transitions now initialize the successor reference from the completed planned phase endpoint (`planned_phase_goal_v1`) while leaving the actual simulator state untouched.  The semantics enter both rollout metadata and simulator hashing.  No target/gate/reward/policy/PPO/QPID/QP/safety/randomization or quota changes were made.  A regression test chains lift, transport, and place while injecting a tolerated 40 mm physical lift error and verifies that the final planned object target remains on the support.
- Corrected A/B result: The identical update-24 checkpoint, bucket, seeds, deterministic actions, and first-terminal full-mesh setup passed `8/8`, with zero fallback decisions, safety failure, collision, drop, QP infeasibility, or timeout.  All terminals were settle (steps `8535--8540`), mean return was `13085.3298`, and raw/evaluation SHA-256 values are `0c7efe396fcf87449d2731ddd78ff48926fc43f2f708ffdb648dcbc6812659e1` and `f123a397627b5baa4c31d0f684510e1b3eb41e45e7d6f1e90f9d7d65f7ee527b`.
- Evaluation load/time: Corrected validation executed `68,320` environment steps in `1114.39 s` (`61.38 env-step/s`).  GPU utilization averaged/peaked at `58.10/65%`, device VRAM at `4383/4595 MiB`, process RSS at `4677/4881 MiB`, temperature at `45.58/57 C`, and peak power was `95.78 W`.
- Verification/next: Focused tensor task/reward/runtime/artifact tests passed `18`; compilation and `git diff --check` passed.  No method-level blocker was found.  Resume point is update 25; updates `25--45` will use the already approved deterministic gzip-level-1 writer, after which the required minimum-200-episode promotion evaluation remains mandatory.

### 2026-07-22 (Order 9 C2 deterministic fast evidence compression)
- Active specification/work package: v0.4 plus the approved Order 9 immutable-generation/exact-replay supplement; Agent J/K C2 execution-time optimization.
- Change: Advanced only the on-policy dataset storage encoding to deterministic gzip level 1. Every record is still validated and serialized as the same JSON object; gzip headers now fix mtime to zero and omit the temporary original filename. Manifests archive the dataset-writer version, compression level, mtime, filename flag, shard SHA-256, and record counts. No raw rollout, split, sample, reward, GAE, PPO, checkpoint, policy/controller interface, promotion gate, or acceptance path changes.
- Timing basis: Recompressing an existing `1,236,107,936`-byte C2 train JSONL stream took `42.79 s` at the previous level 9 and `7.81 s` at level 1. This isolates approximately `35 s` per shard of avoidable compression CPU time; the first complete post-restart generation will provide the end-to-end production measurement.
- Files/tests: Changed `amsrr/training/order9_online_dataset.py`, its deterministic-byte/header test in `tests/unit/training/test_order9_tensor_rollout_artifact.py`, the design supplement, and this worklog. Focused tests passed `6` and then `7`; the complete `isaaclab3` unit suite passed `1196` with `1` skip in `124.12 s` while C2 collection was active. Compilation and `git diff --check` passed.
- Schema/interface/blockers: No dataclass/schema, learned-policy, QPID/QP, safety, or simulator interface change. This is a versioned artifact-encoding optimization. The already-running update 5--24 process retains its imported prior writer; the new encoding begins only at the next clean runner restart, so no in-process behavior changes mid-lineage. No method-level blocker.

### 2026-07-22 (Order 9 C2 update 4 in-memory handoff benchmark)
- Scope/result: Resumed the fail-closed C2 runner at update 4 from checkpoint SHA-256 `4407325a8c5e00b5a5a071e245513b114346129a029eb734902e4b5d0bdb3169`. Concurrent train/validation collection, canonical dataset persistence, exact behavior replay, and one PPO update all passed. The child checkpoint SHA-256 is `7b023ffd21133f9f847cbd712e52e83f85f0ba1c63e1d34df08ec3a337a124aa`; five contiguous updates now account for `327,680` of the required `3,014,656` recorded interactions.
- Immutable generation evidence: Train/validation raw SHA-256 values are `de5325a9b49108165e259ff87028b0d03236fbe7601295061c2d523caf276e1e` and `960b99367430f6540a2e135fd6c6df1844a7dd88d9791bce8f342ea3011ae243`; the `65,536`-record manifest SHA-256 is `9401a3e3522784ded36297f2171cb4573bcf839d284fe2f0163d1d95cad37a2b`. Rollout-only throughput was `6425.21/6242.96 env-step/s`; concurrent collector GPU utilization averaged `65.4/72.53%`, peaked at `99%`, and total VRAM peaked at `11530 MiB`.
- PPO result: Update 4 completed `4/4` epochs and `44` minibatches without KL early stop. Mean KL/clip fraction was `0.00790191/0.108295`; actor/value/total losses were `-0.00759502/69.6951/34.8713`. Exact replay maxima were `1.52588e-5` log probability, `5.66244e-7` recurrent state, and `2.38419e-7` value against tolerance `2e-5`. PPO-only GPU utilization averaged/peaked at `27.38/63%`, VRAM peaked at `2520 MiB`, and RSS peaked at `6107.10 MiB`.
- Timing conclusion: The new generation used the exact persisted-and-validated in-memory bundle (`preloaded_bundle_used=true`). Collection/dataset/PPO wall times were `115.38/302.59/173.05 s`; end-to-end generation time was `596.28 s`, down from update 3's `856.82 s` by `30.4%`. This verifies removal of the redundant compressed-JSON reconstruction without changing the canonical artifacts, training samples, replay gate, optimizer, or checkpoint lineage.
- Verification/blockers: The bounded production runner exited zero and validated all raw, manifest, parent/child, finite-metric, and exact-replay constraints. No method-level blocker or revision was found. Resume point is update 5; the configured 25-update full-mesh checkpoint and final minimum-200-episode promotion evaluation remain required.
...
- Runner production check/update 3: The bounded runner resumed exactly at update 3, selected train bucket 3 and validation bucket 1, and completed one fresh generation in `856.82 s`. Collection/build/training-command times were `112.90/304.00/435.48 s`; the latter included about `259 s` of redundant canonical dataset reload before the `176.49 s` PPO update. Rollout throughput was `6463.49/6451.63 env-step/s`. The update completed `4/4` epochs without early stop at KL/clip `0.00881639/0.128376`, actor/value/total losses `0.000953728/53.1257/26.5952`, and produced checkpoint SHA-256 `4407325a8c5e00b5a5a071e245513b114346129a029eb734902e4b5d0bdb3169`.
- Duplicate-I/O optimization: Newly built generations now retain the exact validated low-level record tuple and pass it directly into the same preflight/trainer implementation after canonical gzip shards and manifest are written and hash-verified. Resumed pre-existing datasets still use the canonical loader. A small tensor-artifact integration test proved equality of every retained and reloaded record. Runner summaries retain aggregate load telemetry but omit only the redundant embedded one-second sample array; complete samples remain in immutable rollout/update metrics.
- Accounting clarification: The runtime product `32768` is per split. Existing dataset/checkpoint lineage counts the paired train+validation generation as `65536` environment interactions, while PPO gradients still use only the `32768` train records. Thus C2 requires 46 updates (`0--45`) and reaches `3,014,656` recorded interactions. Updates 0--3 are contiguous and verified, totaling `262,144`; update 4 is the resume point.
- Files changed: `amsrr/training/order9_pi_l_stage_runner.py`; `amsrr/training/order9_tensor_dataset_builder.py`; `scripts/order9_run_pi_l_ppo_stage.py`; reusable execution entry in `scripts/order9_train_ppo.py`; runner/tensor-artifact tests; design supplement; this worklog. Generated rollout/dataset/checkpoint/TensorBoard artifacts remain ignored and hash-bound.
...
- Tests/commands: New runner tests `3 passed`; focused runner/bucket/online/tensor-artifact/TensorBoard/curriculum tests passed, including `12` after the in-memory handoff; complete unit suites passed `1196` with `1` skip in `94.30 s` and again in `94.39 s`; compilation, `git diff --check`, real update-0--3 lineage/bucket/randomization/raw/dataset/checkpoint validation, and the bounded update-3 production runner passed.
...
- Next steps: Commit the verified in-memory handoff, benchmark it on update 4, then resume through update 45 while retaining state/TensorBoard telemetry; afterward run at least 200 deterministic validation episodes and finalize C2 only if every configured promotion gate passes.
...
- Numerical correction: Set only `optimization.pi_l_ppo.learning_rate` to `1e-5`. A sequential reference rerun from the same promoted C1 parent and unchanged generation-0 rollout completed all `4/4` epochs and `40` optimizer minibatches without early stopping. Mean KL/clip fraction was `0.0094137/0.139402`, final minibatch KL/clip fraction was `0.0110907/0.166873`, and exact replay maxima were `1.52588e-5` log probability, `5.21541e-8` value, and `4.76837e-7` recurrent state. Its update-only wall time was `1290.09 s`, GPU utilization mean/peak `24.91/73%`, and VRAM mean/peak `2494.8/2797 MiB`; it is retained as the numerical reference while the equivalent batched production rerun is prepared.
- Runtime correction: Recurrent replay now batches every active independent episode sequence at the same timestep while preserving its stored initial hidden state, previous action, episode boundary, and recurrent order. The exact gate uses the same execution layout and still rejects any individual record over absolute error `2e-5`. PPO replay provenance advances to `order9_ppo_exact_behavior_replay_v2_batched_sequence`. The training CLI also reuses one path/SHA-verified in-memory dataset bundle across preflight and training, eliminating the prior duplicate full parse.
- Real-data verification: `4096` generation-0 records, forming `2049` sequences active simultaneously at the first timestep, replayed through the batched C1 policy in `31.61 s` including partial gzip parsing/checkpoint setup. The committed full-generation rerun then passed all `32768` records with maximum log-probability/value/recurrent errors `1.14441e-5 / 6.33299e-8 / 5.66244e-7`, below the unchanged gate. Focused policy/online/pipeline/tensor-artifact tests passed `28`; the complete unit suite passed `1193` with `1` skip in `99.41 s`; compilation and `git diff --check` passed.
- Production update 0: Committed implementation `2dffe036d2852f5727207df4d68e0d223d7898cb` produced checkpoint SHA-256 `d8621542646cbae73a8786082054e92a37f0eea8e3aae7f190a646a7c1a52fc7`. It completed `4/4` epochs and `40` optimizer minibatches without KL early stop; mean KL/clip fraction `0.00941373/0.139409` and losses match the slower reference to numerical precision. Update wall time fell from `1290.09 s` to `634.84 s` (`2.03x`). GPU utilization mean/peak was `7.82/50%`, VRAM mean/peak `2426.0/2633 MiB`, and process RSS peak `8529.05 MiB`; CPU-side schema/control-feature reconstruction remains the dominant cost. The slower reference was moved recoverably to `artifacts/p4_full/order9/archive/c2_generation0_sequential_lr1e5_reference_20260722` and is not the active lineage.
- Production generation/update 1: Concurrent train/validation collection from update 0 passed at `6024.81/6115.30 env-step/s` rollout throughput and produced manifest SHA-256 `22badc2d139329785ce91b0e37bf0d86fc0b765b41a07eef5e6b64c4759c458d`. Update 1 completed `4/4` epochs and `40` minibatches without early stop, mean KL/clip fraction `0.00917613/0.136738`, value loss `52.7952`, and update wall time `668.65 s`; checkpoint SHA-256 is `9ffe9b3dcb834db8b0961e718f2508fa6310b8897e1baef4b932851be52d4426`.
- Further execution optimization: Added one-update caching for record-invariant reconstructed actor graph views, centroidal/phase features, and privileged critic inputs. Current-policy recurrence, log probability, value, loss, and gradient remain freshly evaluated on every minibatch. The cached path reproduced the same `4096`-record real-data errors exactly, focused tests passed `20`, and the full unit suite passed `1193` with `1` skip in `97.16 s`. Its full-generation timing will be measured by update 2.
- Method/schema impact: No method-level revision and no policy/action/reward/controller/promotion schema change. This is measured learning-rate calibration plus an algebraically equivalent execution optimization.
- Next steps: Collect generation 1 from the accepted update-0 child, continue one-fresh-generation/one-update training while recording per-generation learning/load telemetry, then perform the minimum-200-episode C2 promotion evaluation after the configured environment-step budget.
...
- Upstream dependencies: Promoted C1 checkpoint, accepted C2 updates `0--24`, corrected `planned_phase_goal_v1` runtime, 8/2 conservative train/validation buckets, exact recurrent behavior replay, QPID/QP/safety stack, full-mesh articulated asset, and typed stage evaluation/finalization pipeline.
- Implemented/evidence: Completed updates `25--45` and the contiguous `0--45` lineage (`3,014,656` interactions); produced final checkpoint SHA-256 `1378d17d...`; measured aggregate rollout throughput `5844.63 env-step/s`; evaluated 100 episodes each on validation buckets 8 and 9; hash-bound the typed aggregate report and formal rejection manifest.
- Result: Bucket 8 passed `100/100`, bucket 9 passed `24/100`, for `124/200 = 0.62`; fallback and safety failures were zero.  C2 was formally rejected only on `minimum_success_rate` (`0.85` required).  The dominant bucket-9 failure is transport-time object orientation drift above `0.20 rad` while both contacts and the `0.05 m` position condition remain valid.
- Files changed: `for_codex/WORKLOG.md` only.  Production evidence is ignored but hash-bound under the C2 artifact tree.
- Schema/interface changes: None.  No reward, gate, randomization, policy action/observation, controller, or training method was changed after observing the failure.
- Tests passed: Focused evaluation/curriculum/pipeline/tensor-artifact/reward/task set `21 passed`; both final real-Isaac evaluators and formal report/finalization commands exited zero.
- Downstream impact/handoff: Do not start C3 and do not treat the final checkpoint as promoted.  Resume only after the user selects a method-level response: audit/select earlier checkpoints on both validation buckets, authorize a bounded extra PPO budget, or revise orientation-related training coverage/reward.  Promotion-gate relaxation is not supported by this evidence.
- Open questions: The current specification does not define which of those recovery paths owns a fixed-budget PPO stage that is safe and throughput-valid but below its success gate.
...
- Implemented/evidence: Accepted generation/update 2 and checkpoint `d74c589d...`; bounded production runner then accepted update 3/checkpoint `4407325a...`; verified update 0--3 contiguous lineage (`262144` paired-split interactions); added fail-closed bucket rotation, concurrent collection, immutable dataset/update orchestration, exact-replay/finite-metric checks, atomic state, artifact-safe resume, and canonical-artifact-preserving in-memory dataset handoff.
- Not implemented: Updates 4--45, the 200-episode C2 promotion evaluation, C2 promotion, C3 parallelism selection, or any later policy family.
...
- Tests passed: Runner `3`; focused related sets including post-handoff `12`; full unit suites `1196 passed, 1 skipped` twice; real artifact validation for update 0--3 and bounded update-3 runner passed.
- Handoff: Resume only from checkpoint SHA-256 `4407325a8c5e00b5a5a071e245513b114346129a029eb734902e4b5d0bdb3169`; benchmark in-memory handoff on update 4, then continue to update 45 followed by a separate promotion evaluation.
