...
- Real-data verification so far: `4096` generation-0 records, forming `2049` sequences active simultaneously at the first timestep, replayed through the batched C1 policy in `31.61 s` including partial gzip parsing/checkpoint setup. Maximum log-probability/value/recurrent errors were `1.14441e-5 / 4.09782e-8 / 3.24100e-7`, below the unchanged gate. Focused policy/online/pipeline/tensor-artifact tests passed `28`; compilation and `git diff --check` passed.
- Method/schema impact: No method-level revision and no policy/action/reward/controller/promotion schema change. This is measured learning-rate calibration plus an algebraically equivalent execution optimization. The full generation-0 batched rerun, complete regression, final checkpoint/hash, and end-to-end timing remain pending at this entry.
- Next steps: Commit the calibrated/batched implementation, archive the slower sequential update-0 reference, rerun update 0 from C1 through the committed batched path, compare its learning metrics, then collect generation 1 from the accepted child.
