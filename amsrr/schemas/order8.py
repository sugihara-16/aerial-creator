...
ORDER8_NATURAL_CONTACT_CONFIG_VERSION = "order8_natural_contact_config_v7"
ORDER8_NATURAL_CONTACT_OBSERVATION_VERSION = "order8_natural_contact_observation_v3"
ORDER8_NATURAL_CONTACT_STEP_EVIDENCE_VERSION = "order8_natural_contact_step_evidence_v3"
ORDER8_NATURAL_CONTACT_RESULT_VERSION = "order8_natural_contact_result_v2"
...
class Order8NaturalContactConfig(SchemaBase):
    ...
    max_cumulative_tangential_slip_m: float = 0.010
...
class Order8NaturalContactStepEvidence(SchemaBase):
    ...
    cumulative_tangential_slip_m_by_link: dict[str, float]
    max_cumulative_tangential_slip_m: float
    object_bottom_clearance_m: float
    transport_distance_m: float
    gripper_object_clearance_m: float
    hard_failure: bool
    ...
    def validate(self) -> None:
        ...
        if any(
            ...
            for value in self.cumulative_tangential_slip_m_by_link.values()
        ):
            raise SchemaValidationError(
                "Order8 cumulative slip values must be finite and non-negative"
            )
...
class Order8NaturalContactResult(SchemaBase):
    ...
    def validate(self) -> None:
        ...
        if any(
            ...
            raise SchemaValidationError(
                "Order8 result cumulative slip values must be finite and non-negative"
            )
        required_pass_gates = (
            self.attempted
