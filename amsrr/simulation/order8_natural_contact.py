...
ORDER8_DEFAULT_SIMULATION_DT_S = 0.005
ORDER8_DEFAULT_ROLLOUT_BUDGET_S = 120.0
# Convex-decomposition contact on the full articulated three-module model can
# run at roughly 0.05x real time on the reference workstation.  Keep the wall
# timeout above the complete 90 s simulation budget; phase timeouts inside the
# planner remain the actual deterministic rollout termination mechanism.
    ...
    *,
    ...
    if _finite_positive(simulation_dt):
        ...
        exact(
            ...
            "object_relative_inward_surface_point_trajectory_with_measured_object_"
            "retarget_and_peak_effort_limited_position_lead_v1",
        )
        exact(
            "order8_natural_contact_contact_force_position_preload_method",
            "post_arrest_object_relative_bounded_geometric_overtravel_before_"
            "independent_torque_ramp_v1",
        )
        for key in (
        ...
        if not math.isclose(
            ...
            failures.append(
                "mismatch:order8_natural_contact_post_qclose_geometric_preload_distance_m"
            )
        if (
            float(
                report.get(
            ...
            failures.append(
                "invalid:order8_natural_contact_post_qclose_geometric_preload_terminal_error_m"
            )
        ...
        if (
            ...
            failures.append(
                "invalid:order8_natural_contact_post_qclose_geometric_preload_active_step_count"
            )
        if (
            report.get(
        ...
        exact(
            ...
            "ik_position_trajectory_and_simulator_consistent_below_ak40_10_"
            "configured_speed_limit_v1",
        )
        exact(
            "order8_natural_contact_joint_position_reference_mode",
            "online_object_relative_mesh_region_measured_q_receding_"
            "differential_ik_until_qclose_then_absolute_measured_hold_with_"
            "independent_torque_bias_v7",
        )
        maximum_joint_position_lead = report.get(
            "order8_natural_contact_max_joint_position_command_lead_rad"
        )
        if (
            not _finite_non_negative(maximum_joint_position_lead)
            or float(maximum_joint_position_lead)
            > expected_joint_controller.max_position_command_lead_rad + 1.0e-9
        ):
            failures.append(
        ...
        exact(
            ...
            "clear_authored_mesh_tangential_precenter_then_simultaneous_"
            "normal_joint_close_v22",
        )
        exact(
        ...
        true("order8_natural_contact_contact_mesh_precenter_complete")
        precenter_dwell = report.get(
            "order8_natural_contact_contact_mesh_precenter_dwell_s"
        )
        if not (
            _finite_non_negative(precenter_dwell)
            and float(precenter_dwell) >= config.contact_stall_dwell_s - 1.0e-9
        ):
            failures.append(
        ...
        exact(
            ...
            "none_final_object_relative_base_pose_reached_before_joint_only_close_v1",
        )
        exact(
            "order8_natural_contact_contact_centering_joint_motion_mode",
            "all_docks_simultaneous_surface_region_joint_only_close_at_settled_base_v1",
        )
        exact(
        ...
        exact(
            ...
            "quasi_static_measured_shape_no_commanded_joint_motion_"
            "compensation_v1",
        )
        false(
    ...
    exact(
        ...
        "simultaneous_selected_anchor_sampled_mesh_surface_region_proximity_"
        "full_object_relative_settle_filtered_normal_arrest_and_per_anchor_"
        "kinematic_chain_dock_load_v16",
    )
    exact(
    ...
    exact(
        ...
        "disabled_continuous_bounded_creep_until_simultaneous_qclose_v2",
    )
    false(
    ...
    if closure_reason != "dynamic_simultaneous_surface_region_arrest":
    ...
    if (
        ...
        or float(stall_dwell) < config.contact_stall_dwell_s - 1.0e-9
    ):
    ...
    if not (
        ...
        and all(
            _finite_non_negative(value)
            and float(value) >= config.contact_stall_dwell_s - 1.0e-9
    ...
    if isinstance(stall_speeds, dict) and any(
        float(value) > config.contact_stall_anchor_speed_threshold_mps + 1.0e-9
        for value in stall_speeds.values()
        if _finite_non_negative(value)
    ):
        failures.append(
            "above:order8_natural_contact_contact_stall_anchor_speed_mps_by_anchor"
        )
    actuator_specs = physical_model.metadata.get("joint_actuator_specs", {})
    dock_spec = (
    ...
    exact(
        ...
        "authored_mesh_surface_sample_rigid_point_jacobian_for_contact_"
        "translation_and_consistent_wrench_application_v2",
    )
    ...
    ramp_elapsed_by_anchor = report.get(
        "order8_natural_contact_contact_force_ramp_elapsed_s_by_anchor"
    )
    if not (
        isinstance(ramp_elapsed_by_anchor, dict)
        and set(ramp_elapsed_by_anchor) == expected_anchor_metric_ids
        and all(
            _finite_non_negative(value)
            and float(value) >= config.contact_force_ramp_s - 1.0e-9
            for value in ramp_elapsed_by_anchor.values()
        )
    ):
        failures.append(
    ...
    exact(
        ...
        "requested_unclipped_limited_and_isaac_target_computed_applied_v1",
    )
    ...
    if not isinstance(telemetry_maxima, dict):
        ...
        for key in (
            ...
            if (
                ...
                or float(value) > peak_torque_nm + 1.0e-6
            ):
                failures.append(
    ...
    if monitor_result is not None:
        ...
        if set(monitor_result.cumulative_tangential_slip_m_by_link) != set(
            expected_selected_link_ids
        ):
            failures.append(
                "mismatch:order8_natural_contact_monitor_result.cumulative_slip_links"
            )
        elif any(
            value > config.max_cumulative_tangential_slip_m + 1.0e-12
            for value in monitor_result.cumulative_tangential_slip_m_by_link.values()
        ):
            failures.append(
                "above:order8_natural_contact_monitor_result.cumulative_slip"
            )
        exact(
