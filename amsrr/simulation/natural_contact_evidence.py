...
class NaturalContactEvidenceMonitor:
    ...
    _cumulative_slip_m_by_link: dict[str, float] = field(
    ...
    def reset(self) -> None:
        ...
        self._cumulative_slip_m_by_link = {}
    ...
    def observe(
        ...
        for link_id, metrics in selected_physical.items():
            if maintained_contact_active and link_id in selected_contact_link_ids:
                self._cumulative_slip_m_by_link.setdefault(link_id, 0.0)
                self._cumulative_slip_m_by_link[link_id] += (
                    metrics.max_slip_speed_mps * dt_s
                )
        max_cumulative_slip = max(
            self._cumulative_slip_m_by_link.values(), default=0.0
        )
        ...
        if (
            maintained_step_max_slip
            > self.config.max_tangential_slip_speed_mps + _EPS
        ):
            self._record_failure("selected_contact_slip_speed_limit_exceeded")
        if (
            max_cumulative_slip
            > self.config.max_cumulative_tangential_slip_m + _EPS
        ):
            self._record_failure("selected_contact_cumulative_slip_limit_exceeded")
        ...
        if phase == Order8NaturalContactPhase.CONTACT_ACQUISITION:
            if not observation.grasp_confirmation_ready:
                ...
                self._contact_dwell_s = 0.0
                self._contact_dwell_gap_s = 0.0
            if self._contact_dwell_s + _EPS >= self.config.contact_dwell_s:
        ...
        gate_results = {
            ...
            "selected_slip_speed_safe": (
                step_max_slip
                <= self.config.max_tangential_slip_speed_mps + _EPS
            ),
            "selected_cumulative_slip_safe": (
                max_cumulative_slip
                <= self.config.max_cumulative_tangential_slip_m + _EPS
            ),
        ...
        return Order8NaturalContactStepEvidence(
            ...
            cumulative_tangential_slip_m_by_link=dict(
                self._cumulative_slip_m_by_link
            ),
            max_cumulative_tangential_slip_m=max_cumulative_slip,
