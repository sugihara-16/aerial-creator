...
ORDER8_SIMPLE_CLOSURE_JOINT_SPEED_RADPS = 0.020
...
def _torque_bias_limit_with_peak_window(
    *,
    ...
    # Avoid a sub-picotorque interpolation residue being reported as a bridge
    # clip exactly at the continuous-rating handoff.
    if scheduled <= continuous + 1.0e-12:
        return continuous
...
def _payload_feedforward_scale_for_phase(
    ...
    *,
    ...
    """Blend known payload support after verified grasp and during release.

    LIFT must create the initial upward friction that unloads the floor.  A
    scale driven only by object COM rise is circular: the object cannot rise
    until QPID accounts for its weight.  Once the independent nominal and raw
    safety dwell gates authorize LIFT, ramp the known TaskSpec payload mass in
    with the same duration used for contact-motion entry.  Measured COM rise is
    retained as an audit and a monotonic lower bound, never as contact wrench.
    RELEASE removes compensation smoothly over the configured duration.
    """
    ...
    if phase == Order8NaturalContactPhase.LIFT:
        if measured_lift_transfer_scale is None:
            raise SchemaValidationError(
                "Order8 lift payload feed-forward requires measured load-transfer scale"
            )
        ...
        commanded_transfer = min(1.0, elapsed / duration)
        return max(commanded_transfer, measured)
    if phase in {
        Order8NaturalContactPhase.TRANSPORT,
        Order8NaturalContactPhase.PLACE,
    }:
...
def _contact_motion_entry_speed_scale(
    ...
    *,
    ...
    """Hold during payload transfer, then ramp maintained-contact motion.

    LIFT first keeps the measured grasp pose fixed for one transition interval
    while known-payload feed-forward rises from zero to full support.  Only
    then does centroidal lift motion ramp over a second interval.  TRANSPORT
    and PLACE have no new payload transfer, so their motion ramps immediately.
    """
    ...
    if phase == Order8NaturalContactPhase.LIFT:
        return min(1.0, max(0.0, (elapsed - duration) / duration))
    if phase in {
        Order8NaturalContactPhase.TRANSPORT,
        Order8NaturalContactPhase.PLACE,
    }:
...
def run_order8_isaac_runtime(
    *,
    ...
    # separate high-friction material is bound only to the two selected Dock
    # collision surfaces.  This supplies the friction margin needed to carry a
    # 1 kg payload without increasing the AK40-10 continuous torque bias, and
    # avoids making the object artificially difficult to move on the floor
    # during joint-only closure.
    ...
    if state_trace_replay_path is not None:
        ...
        return {
            ...
            "order8_state_trace_replay_advances_physics": False,
    ...
    robot_object_contact_capacity = max(
        256 if diagnostic_only else 64,
    ...
    for selection in selections:
        ...
        if not matching_bounds:
            raise RuntimeError(
                "Order8 selected anchor has no matching URDF collision-mesh "
    ...
    def apply_commands(
        ...
        *,
        ...
        if feedforward_scale > 0.0 or admittance_active:
        ...
        if feedforward_scale > 0.0:
            ...
            payload_coupling = _natural_contact_payload_coupling(
    ...
    # q_close is the measured configuration at simultaneous terminal-joint
    # load arrest.  Order 8 does not append a mesh-tracking IK preload after
    # that event; normal force is established only through the independent
    # bounded torque-bias channel.
    ...
    post_qclose_geometric_preload_completion_source: str | None = (
        "not_applicable_measured_qclose_then_independent_torque_bias_v1"
    )
    ...
    for _ in range(max_steps):
        ...
        contact_preload_ready = bool(
            ...
            and contact_force_scale >= 1.0 - 1.0e-12
            and all(
                current_filtered_gripper_surface_clearance_speed_mps_by_anchor[
                    anchor_id
                ]
                <= float(config.contact_stall_anchor_speed_threshold_mps)
                and per_anchor_influential_dock_load_nm[anchor_id]
                >= contact_stall_selected_joint_load_threshold_nm
                for anchor_id in selected_anchor_ids
            )
        )
        ...
        contact_command_ready = bool(
            contact_preload_ready and prelift_relative_motion_settled
        )
        observation = Order8NaturalContactObservation(
            ...
            # Raw contact dwell begins only after the full-force preload,
            # measured closed-configuration rebase, ordinary-gain restore,
            # and non-privileged full relative-motion settle.  This prevents
            # controller-transition motion from being mislabeled as
            # maintained-contact slip after grasp acquisition.
        ...
        if contact_preload_ready and not contact_yield_grasp_pose_rebased:
            if contact_axial_hold_base_pose is None:
                raise RuntimeError(
                    "Order8 full-force preload was acquired without an axial hold pose"
                )
            # Rebase the normal centroidal controller from non-privileged
            # full-force preload evidence before restoring pose P/I.  Raw
            # contact truth is not used for this control-mode transition.
            contact_yield_requested = False
            contact_admittance_requested = False
            # The compliant Dock drive is needed while discovering q_close.
            # Restore its ordinary impedance smoothly, but retain the bounded
            # object-relative geometric-preload position target.  Replacing
            # that target with measured q would erase the normal-force margin
            # immediately before payload transfer.  The independent
            # wrench-bias command and AK40-10 torque/current/speed audits remain
            # unchanged.
            contact_yield_joint_drive_requested = False
        ...
        if phase == Order8NaturalContactPhase.RESET:
            ...
            # configuration.  Hold that achieved geometry absolutely while
            # the independent offset-torque channel ramps contact wrench.
            # Re-encoding the same wrench as a position offset double-counted
            # it and saturated the virtual servo before physical contact.
        ...
        if (
            ...
            # independent contact-wrench torque ramp stays at zero.
            joint_position_reference_by_id = {
                joint_id: float(position)
                for joint_id, position in zip(
                    joint_vector.joint_ids,
            ...
            if (
                ...
                # sample, is the q_close held during force ramp and carriage.
                qclose_base_pose_snapshot = base_root_pose
        ...
        unmasked_contact_force_scale_by_anchor = {
            anchor_id: _contact_force_scale_for_phase(
                phase=phase,
                ramp_elapsed_s=(
                    nonprivileged_contact_force_ramp_elapsed_s_by_anchor[anchor_id]
                ),
                ramp_duration_s=float(config.contact_force_ramp_s),
            )
            for anchor_id in selected_anchor_ids
        }
        contact_force_scale_by_anchor = {
        ...
        if planner.phase == Order8NaturalContactPhase.APPROACH:
            ...
            elif not post_qclose_geometric_preload_complete:
                contact_motion_subphase = "object_relative_geometric_preload"
            else:
                contact_motion_subphase = "force_ramp_hold"
        else:
        ...
        progress_message = (
            ...
            + f" cumulative_slip={last_evidence.max_cumulative_tangential_slip_m:.4f}m"
        ...
        if (
            ...
            break
        if planner.phase == Order8NaturalContactPhase.COMPLETE:
            break
        ...
        last_control_result = low_level.compute(
            joint_vector,
            tasks,
        ...
        # q_close is held directly after the simultaneous load arrest.  The
        # legacy geometric-preload branch is intentionally inactive; contact
        # force is added only through the independent torque-bias ramp.
        ...
        # q_close is an absolute measured joint configuration.  Neither a
        # residual anchor-pose error caused by compliant graph constraints nor
        # the independently ramped torque bias may ratchet that reference.
        # Release returns directly toward the measured open joint
        # configuration captured at closure onset.
    ...
    if (
        ...
        # failing episode into a pass.
        terminal_observation = replace(
            observation, phase=Order8NaturalContactPhase.COMPLETE
        )
    ...
    report = {
        ...
        "order8_natural_contact_contact_yield_grasp_rebase_method": (
            "nonprivileged_full_force_preload_measured_full_6d_centroidal_"
            "with_measured_qclose_preserved_then_pi_impedance_restore_v5"
        ),
        ...
        "order8_natural_contact_joint_position_reference_mode": (
            ...
            "load_qclose_then_absolute_measured_hold_with_independent_torque_"
            "bias_and_direct_open_return_v10"
        ),
        ...
        "order8_natural_contact_contact_motion_sequence": (
            ...
            "fixed_velocity_close_until_simultaneous_load_qclose_v24"
        ),
        "order8_natural_contact_contact_mesh_precenter_method": (
            "selected_authored_mesh_samples_inside_surface_region_at_"
            "clearance_then_normal_close_v2"
        ),
        ...
        "order8_natural_contact_contact_slip_enforcement_phase": (
            "after_verified_two_contact_grasp_dwell_until_planned_release_v1"
        ),
        ...
        "order8_natural_contact_qpid_joint_motion_assumption": (
            "quasi_static_measured_shape_no_commanded_joint_motion_" "compensation_v1"
        ),
        ...
        "order8_natural_contact_contact_force_joint_impedance_mode": (
            "measured_qclose_position_hold_plus_continuous_rated_selected_"
            "anchor_jacobian_transpose_offset_torque_v5"
        ),
        ...
        "order8_natural_contact_contact_force_position_preload_method": (
            "disabled_measured_qclose_then_independent_torque_ramp_v2"
        ),
        ...
        "order8_natural_contact_latched_joint_position_hold_method": (
            "absolute_measured_qclose_with_independent_continuous_torque_bias_v5"
        ),
        ...
        "order8_natural_contact_contact_force_hold_settle_gate_method": (
            "all_selected_sampled_mesh_proximity_low_pass_signed_mesh_"
            "clearance_rate_and_per_anchor_kinematic_chain_applied_dock_load_v7"
        ),
        "order8_natural_contact_contact_force_hold_settle_raw_contact_input": False,
        "order8_natural_contact_grasp_confirmation_method": (
            "full_force_nonprivileged_mesh_proximity_low_pass_signed_mesh_"
            "clearance_rate_per_anchor_chain_load_and_full_relative_motion_"
            "settle_then_privileged_safety_interlocked_raw_contact_dwell_v9"
        ),
        ...
        "order8_natural_contact_post_qclose_geometric_preload_method": (
            "not_applicable_measured_qclose_then_independent_torque_bias_v4"
        ),
        "order8_natural_contact_post_qclose_settle_method": (
            "measured_absolute_joint_position_rebase_zero_velocity_target_"
            "with_force_ramp_held_zero_until_joint_base_and_mesh_rate_dwell_v1"
        ),
        ...
        "order8_natural_contact_contact_wrench_application_mapping": (
            "selected_anchor_frame_jacobian_transpose_offset_torque_v3"
        ),
        ...
        "order8_natural_contact_selected_gripper_material_method": (
            "selected_dock_rigid_body_stronger_inherited_collision_material_v2"
        ),
        ...
        "order8_natural_contact_contact_vector_telemetry_role": (
            "privileged_diagnostic_only_not_actor_or_qpid_input_v1"
        ),
        ...
        "order8_natural_contact_payload_feedforward_method": (
            "verified_grasp_time_ramped_known_payload_qpid_coupling_with_"
            "measured_free_object_pose_and_lift_audit_v5"
        ),
        ...
        "order8_natural_contact_payload_load_transfer_observation": (
            "monotonic_measured_object_com_z_rise_since_qclose_v1"
        ),
        "order8_natural_contact_payload_load_transfer_driver": (
            "phase_elapsed_ramp_after_nominal_and_privileged_grasp_dwell_v1"
        ),
        ...
        "order8_natural_contact_contact_motion_entry_speed_ramp_method": (
            "lift_payload_feedforward_hold_then_linear_motion_ramp_and_"
            "maintained_contact_phase_entry_ramp_v5"
        ),
...
def _replay_order8_state_trace(
    ...
    *,
    ...
    """Replay kinematic states against wall time, dropping late GUI frames."""
            ...
            if frame_index > rendered_index:
                ...
                if (
                    ...
                    print(
                        ...
                        "write_error="
                        f"{maximum_joint_write_error_rad:.6f}rad",
                        end="",
                        flush=True,
                    )
    ...
    for module_id, robot in sorted(robots.items()):
        ...
        robot.write_joint_velocity_to_sim_index(
            velocity=torch.tensor(
                [module_state["joint_velocities_radps"]],
                dtype=torch.float32,
                device=sim.device,
            )
        )
